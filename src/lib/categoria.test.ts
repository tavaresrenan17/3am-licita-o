...
describe("classificação de categoria pelo objeto", () => {
  it("reconhece as famílias de obra com acentuação real do PNCP", () => {
    expect(classificarCategoria("Execução de obra de pavimentação asfáltica em vias urbanas")).toBe(
      "Pavimentação",
    );
    ...
    expect(classificarCategoria("Elaboração de projeto executivo de creche municipal")).toBe(
      "Serviços de engenharia",
    );
    expect(classificarCategoria("Reforma e ampliação de unidade básica de saúde")).toBe("Reforma");
    expect(classificarCategoria("Contratação de empresa para construção de escola municipal")).toBe(
      "Obra nova",
    );
  });
