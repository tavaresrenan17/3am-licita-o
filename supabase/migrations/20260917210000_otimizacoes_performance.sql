-- ==============================================================================
-- 3AM LICITAÇÃO - OTIMIZAÇÕES CRÍTICAS DE BANCO DE DADOS E PERFORMANCE
-- ==============================================================================
-- 1. Criação de coluna gerada e índice GIN Trigram consolidado para busca rápida
-- 2. Eliminação do double-update e desoneração de locks em pncp_merge_page
-- 3. Otimização de contadores atômicos na tabela sincronizacoes
-- 4. Prevenção de inchaço (bloat) em licitacoes_historico
-- 5. Otimização da RPC buscar_licitacoes
-- ==============================================================================

-- 1. Garante extensão de trigramas no schema extensions
create extension if not exists pg_trgm with schema extensions;

-- 2. Coluna gerada unificada para busca textual rápida
do $$
begin
  if not exists (
    select 1 from information_schema.columns
     where table_schema = 'public'
       ...
       and column_name = 'busca_texto'
  ) then
    alter table public.licitacoes
      add column busca_texto text
      generated always as (
        coalesce(orgao, '') || ' ' || coalesce(objeto, '') || ' ' || coalesce(municipio, '')
      ) stored;
  end if;
end $$;

-- Índice Trigram sobre a coluna unificada
create index if not exists licitacoes_busca_texto_trgm_idx
  on public.licitacoes using gin (busca_texto extensions.gin_trgm_ops);

-- Índice em documentos_licitacao para otimizar os filtros de com_edital/com_projeto/com_orcamento
create index if not exists documentos_licitacao_tipo_idx
  on public.documentos_licitacao (licitacao_id, tipo_documento);

-- 3. Função otimizada pncp_merge_page (sem double-update e sem subquery count(*))
...
-- 4. Função otimizada buscar_licitacoes usando a coluna gerada busca_texto
...
analyze public.ingestao_segmentos;
analyze public.sincronizacoes;
