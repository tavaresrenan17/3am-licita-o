-- Um 5xx do PNCP deve adiar somente o segmento afetado. Sem cooldown, o mesmo
-- segmento volta imediatamente ao início da fila e impede outras modalidades.
alter table public.ingestao_segmentos
  add column if not exists proxima_tentativa_em timestamptz;

create index if not exists ingestao_segmentos_prontos_idx
