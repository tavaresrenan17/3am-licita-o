-- Filtro por raio geográfico calculado no banco, pelo código IBGE.
--
-- Antes, a tela localizava a cidade pelo NOME num dicionário de 56 cidades e
-- filtrava só os 25 itens da página já carregada. Medido em 22/09/2026 contra
-- as 5.195 licitações abertas: 50% das que estavam dentro do raio sumiam, e a
-- paginação continuava mostrando o total sem raio.
--
-- Agora o PNCP já entrega `codigo_ibge` em 100% das licitações. A distância sai
-- dele e da tabela oficial de municípios, e o filtro roda antes do LIMIT/OFFSET,
-- então total, paginação e contagem passam a obedecer o raio.
--
-- Fonte das coordenadas: IBGE, via github.com/kelvins/municipios-brasileiros
-- (licença MIT), 5.571 municípios (malha 2024). Distância em linha reta (haversine), não
-- rodoviária.
...
comment on table public.municipios is
  'Municípios do IBGE com coordenadas da sede. Base do filtro por raio da busca.';

alter table public.municipios enable row level security;
revoke all on table public.municipios from public, anon, authenticated;

insert into public.municipios (codigo_ibge, nome, uf, lat, lon) values
...
on conflict (codigo_ibge) do update
  set nome = excluded.nome, uf = excluded.uf, lat = excluded.lat, lon = excluded.lon;

analyze public.municipios;

-- Haversine em km, linha reta. O least() protege o asin de arredondamentos
-- que passam 1 por um ulp quando os pontos são antípodas.
...
-- Nula quando o município não é conhecido ou quando não há origem: a busca
-- trata "sem distância" como "fora de qualquer raio", nunca como zero.
create or replace function public.distancia_municipio_km(
  p_codigo_ibge text,
...
set search_path = pg_catalog, pg_temp
as $$
  select public.distancia_km(p_lat, p_lon, m.lat, m.lon)
...
-- Lê `origem_ibge` e `raio_km` de p_filtros. Sempre devolve exatamente uma
-- linha. Origem sem raio só calcula a distância para exibir; raio sem origem,
-- ou origem que não existe no IBGE, é erro de quem chamou e não vira "sem filtro".
create or replace function public.resolver_origem_raio(p_filtros jsonb)
returns table (lat double precision, lon double precision, raio_km double precision)
language plpgsql
stable
set search_path = pg_catalog, pg_temp
as $$
declare
  v_ibge text := nullif(btrim(coalesce(p_filtros->>'origem_ibge', '')), '');
  v_raio double precision :=
    nullif(btrim(coalesce(p_filtros->>'raio_km', '')), '')::double precision;
begin
  if v_raio is not null and v_raio <= 0 then
    v_raio := null;
  end if;

  if v_ibge is null then
    if v_raio is not null then
      raise exception 'raio_km exige origem_ibge' using errcode = '22023';
    end if;
  ...
  return query
    select m.lat, m.lon, v_raio
      from public.municipios m
     where m.codigo_ibge = v_ibge;

  if not found then
    raise exception 'origem_ibge desconhecida: %', v_ibge using errcode = '22023';
  end if;
end;
$$;

revoke all on function public.resolver_origem_raio(jsonb) from public, anon, authenticated;
revoke all on function public.distancia_municipio_km(text, double precision, double precision)
  from public, anon, authenticated;

-- Busca lexical: mesmo contrato de 20260918130000, mais raio e distancia_km.
...
declare
  ...
  v_geo record;
begin
  select * into v_geo from public.resolver_origem_raio(p_filtros);
  ...
  with filtrados as (
    select l.*
     ...
     where (
       ...
       and (v_geo.raio_km is null
            or public.distancia_municipio_km(l.codigo_ibge, v_geo.lat, v_geo.lon) <= v_geo.raio_km)
  ),
  ...
  pagina as (
    select f.*,
           ...
           round(public.distancia_municipio_km(f.codigo_ibge, v_geo.lat, v_geo.lon)::numeric, 1)
             as distancia_km
      from filtrados f
     order by
...
-- Busca híbrida: mesmo contrato de 20260920120000, mais raio e distancia_km.
  ...
  return (
    ...
    ordenado as (
      select e.*,
             ...
             round(public.distancia_municipio_km(e.codigo_ibge, v_geo.lat, v_geo.lon)::numeric, 1)
               as distancia_km,
...
-- `create or replace` preserva os grants existentes; reafirmados por clareza.
