# Design Spec: Seleção em Lote de Licitações, Sincronização de Documentos e Painel Alexandria
...
4. **Redirecionar automaticamente para a página Alexandria** (`/alexandria`), um novo hub focado na análise cautelosa, detalhada e orientada por IA de cada licitação com acervo documental disponível.
...
- **Barra de Ações Flutuante (Floating Bulk Action Bar):**
  ...
  - Contém:
    ...
    - Botão de atalho: `Ir para Alexandria`.
...
- Ao clicar em `Sincronizar Documentos`:
  ...
  - Ao concluir:
    ...
    - Redirecionamento automático para a rota `/alexandria?ids=...`.

### 2.3. Painel Alexandria (`/alexandria`)
- **Conceito:** A "Biblioteca de Alexandria" das licitações — um ambiente limpo, focado e livre de distrações para auditoria, leitura de editais e acionamento de IA.
- **Estrutura:**
  - Integrado ao `AppShell` com novo item de navegação no menu lateral e atalho de teclado (`g a`).
  - Cabeçalho executivo com contadores: *Total em Alexandria*, *Documentos Prontos*, *Análises de IA Geradas*, *Investimento Estimado Total*.
...
- **Nova rota:** `src/routes/alexandria.tsx` registrada no TanStack Router (`createFileRoute("/alexandria")`).
- **Menu Lateral (`src/components/AppShell.tsx`):**
  - Inclusão do item `{ to: "/alexandria", label: "Alexandria", icon: Library }`.
...
- **Server Function de listagem para Alexandria (`obterLicitacoesAlexandriaFn`):**
...
- `useSincronizarDocumentosLote()`:
  - Mutation do TanStack Query com invalidação automática de `['licitacoes']`, `['metricas']` e `['alexandria']`.
- `useLicitacoesAlexandria(params)`:
...
2. **Testes de Rota e Navegação:**
   - Verificação de renderização da rota `/alexandria`.
...
3. **Validação E2E no Navegador:**
   ...
   - Chegada e navegação na página Alexandria.
